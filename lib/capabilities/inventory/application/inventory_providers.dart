...
/// ★★★ **خيارُ صرفٍ — نوعٌ له سجلٌ في دفتر اليوم** ([`DEBT-86`]).
///
/// ★ **ومفتاحُه [itemKey] كما هو في الدفتر** — ⟵ **مجرَّداً كان (وارد عدداً)
/// أو مركّباً** (`{النوع} - {الرعوي} - جونية رقم N` — `ADR-0007`).
final class StockOption {
  ...
  /// ★★ **مفتاحُ النوع في الدفتر** — ⛔ **لا معرّفُ سجلِ نوعٍ بالضرورة.**
  final String itemKey;

  /// اسمُه المعروض **كما كُتب في الدفتر** — ★ **نسخةٌ تاريخيةٌ مقصودة**
  /// (`ADR-0007` القاعدة 4).
  final String itemName;

  /// وحدتُه — ★ **وحدةُ الرصيد هي وحدةُ النوع** (`GR-19`).
  final ItemUnit unit;

  /// رصيدُه اليوم.
  final StockQuantity balance;

  /// ★ وزنُ الحبة **من سجل النوع إن وُجد** — ⛔ **عرضٌ محضٌ** (`AM-012` §4.4).
  final double? pieceWeightGrams;
}

/// ★★★ **خياراتُ الصرف — مبنيّةً من أرصدة الدفتر** ([`DEBT-86`] · `ADR-0007`).
///
/// ═══════════════════════════════════════════════════════════════════════
/// ⛔⛔★★★ **ولماذا من الدفتر لا من كتالوج الأنواع — [`DEBT-86`] مقيسٌ على
/// `Pixel_6_API_36` (2026-09-02):** ★ **سطرُ الجونية يدخل المخزن بمفتاحٍ
/// مركّب** (`سلة - جونية رقم 1`) ⛔ **لا بمعرّف سجل نوع**، ★ **ومنسدلاتُ
/// التوزيع والبيع النقدي والسحبيات كانت تُبنى من `items` بمعرّفاتِ `itemId`**
/// ⟹ ⛔⛔ **فلا يظهر المفتاحُ المركّب في أي قائمة اختيار قطّ**، ★ **ولو
/// أُدخِل بمعرّف النوع المجرَّد ردّت السحابة «الكمية غير كافية»** — ⟵ **إذ
/// لا رصيدَ على ذلك المفتاح.** ★★ **مقيسٌ في الاتجاهين:** **«عتود» الواردُ
/// في جونية يُعرَض في «مخزون اليوم» ⛔ ولا يُوزَّع**، ✅ **و«سلة» الواردةُ
/// عدداً تُوزَّع فوراً.**
///
/// ⛔⛔★★★ **وهو نقضٌ لنصِّ `ADR-0007` نفسِه** — ★ **الخيار 2 رُفض حرفياً لأن
/// «المستخدم لا يرى الفرق في قوائم الاختيار»**، ⟵ **والخيارُ المعتمَد وعد
/// بأن «التمييز مرئي في كل قائمة اختيار وكل شاشة مخزون»**: ⛔ **والقائمةُ
/// لم تكن تحمله.**
///
/// ★★ **ونظيرُ [pricingRowsProvider] حرفياً** — ⟵ **وهو ما جعل شاشةَ
/// التسعير وحدَها تنجو من العطل**: ★ **بُنيت من الرصيد يوم كُتبت** (`FR-M9-02`)،
/// ⛔ **ولم تُبنَ من قائمة الأنواع.**
/// ═══════════════════════════════════════════════════════════════════════
///
/// ⚠️⚠️★★★ **والرصيدُ الصفر يبقى خياراً هنا** — ⛔ **بخلاف شاشة التسعير**:
/// ★ **تعديلُ مستندٍ استنفد رصيدَ نوعه يجعل رصيدَه اليومَ صفراً بالضبط**
/// (**الداخل = الخارج**)، ⟹ ⛔⛔ **وإسقاطُه كان يفتح نموذجَ التعديل بمنسدلٍ
/// لا يحوي النوعَ المختار** — ★ **وهو حرفياً عَرَضُ [`DEBT-88`]**: **حقلٌ
/// يبدو فارغاً والكميةُ والسعرُ مبذوران بجانبه.** ★ **والمتبقّي في النصِّ
/// يقول «(0 حبة)» صراحةً** ⟵ **فلا يُغري بكتابة كمية.**
///
/// ⛔★★ **ونوعٌ معطَّلٌ في الكتالوج لا يظهر** (`FR-M5-10`) — ★ **والمفتاحُ
/// المركّب لا سجلَ له فيظهر دائماً**: ⟵ **وهو نفسُ ما تقرّره السحابة في
/// `withLedgerItems`** (`isActive: true` **لكل مفتاحٍ يُستكمَل من الدفتر**)،
/// ⛔ **فلا تعرض الشاشةُ ما ترفضه العملية ولا تُخفي ما تقبله.**
///
/// ⛔⛔★★ **ولا ترشيحَ بالمصدر هنا** — ★ **الرصيدُ مُقيَّدٌ بمصدره في الاستعلام
/// أصلاً** (`A-01`): ⟵ **ووجودُ رصيدٍ إثباتُ انتماءٍ لا افتراض**، ⛔ **وإعادةُ
/// الفحص من `sourceIds` كانت تُخفي مخزوناً قائماً متى فُكَّ ربطُ النوع بمصدره
/// بعد وروده** — ★ **والدفترُ هو الحَكَم** (`ADR-0008`).
final stockOptionsProvider = Provider.family<List<StockOption>, StockQuery>(
  (Ref ref, StockQuery query) {
    final List<ItemDailyBalanceCard> balances =
        ref.watch(todayStockProvider(query)).value ??
            const <ItemDailyBalanceCard>[];
    final List<ItemCard> catalog =
        ref.watch(itemsProvider).value ?? const <ItemCard>[];
    final Map<String, ItemCard> byId = <String, ItemCard>{
      for (final ItemCard item in catalog) item.itemId: item,
    };
    return <StockOption>[
      for (final ItemDailyBalanceCard balance in balances)
        if (byId[balance.itemKey]?.isActive ?? true)
          StockOption(
            itemKey: balance.itemKey,
            // ★★★ **والاسمُ المركّب هو المعروض** — [`ADR-0007`] · [`DEBT-86`]:
            //    ⟵ **فجونيتان من نوعٍ واحد لا تُعرَضان خيارين متطابقين.**
            ...
            pieceWeightGrams: byId[balance.itemKey]?.pieceWeightGrams,
          ),
    ];
  },
);
    ...
    for (final ItemDailyBalanceCard card in stock)
      ...
      if (card.balance.isPositive)
        PricingRow(
          ...
          // ★★★ **والاسمُ المركّب هو المعروض** — [`ADR-0007`] · [`DEBT-86`]:
          //    ⟵ **فجونيتان من نوعٍ واحد لا تُسعَّران في صفّين متطابقين.**
          itemName: ledgerItemDisplayName(
            itemKey: card.itemKey,
...
/// ★★★ **جواني اليوم — بمصدرٍ واحد أو بكل المصادر** (`AM-012` §2).
///
/// ═══════════════════════════════════════════════════════════════════════
/// ⛔⛔★★★ **واستعلامٌ مقيَّدٌ لكل مصدرٍ ثم دمج** — ★ **نظيرُ
/// [countedIntakeListProvider] حرفياً** ⛔ **لا استعلامٌ واحدٌ غيرُ مقيَّد:**
/// ⟵ **فشرطُ قراءة `sacks` يعتمد `resource.data`**، ★ **والسردُ يُقيَّم على
/// قيود الاستعلام لا على كل مستند** ⟹ ⛔ **فاستعلامٌ لا يُقيّد `sourceId`
/// يُرفَض كاملاً ولو بنطاقٍ شامل** (`IQ-024` · `DEBT-40` · `WU-008`).
///
/// ★★ **ولماذا صار للجواني قائمةُ «كل المصادر» أصلاً** — `AM-012` §2:
/// ⟵ **دُمجت شاشتاه مع «الوارد عدداً» في شاشةٍ بتبويبين ومرشِّحٍ مشترك**،
/// ★ **والمرشِّحُ يقبل «الكل» في التبويبين معاً بقرار المالك** (§2 السؤال ②)
/// ⛔ **ولا مرشِّحَ مستقلٌّ لكل تبويب.**
///
/// ⛔⛔★★ **والقراءةُ وحدها هي ما اتّسع** — ★ **والكتابةُ تبقى على مصدرٍ
/// واحدٍ يُختار في النموذج** (`A-01`: ⛔ **لا جمعَ بين مصدرين في أي عملية**).
/// ═══════════════════════════════════════════════════════════════════════
final sackListProvider =
    Provider.family<AsyncValue<List<SackCard>>, SourceListQuery>(
  (Ref ref, SourceListQuery query) => combineAsyncLists<SackCard>(
    <AsyncValue<List<SackCard>>>[
      for (final String sourceId in listedSourceIds(ref, query))
        ref.watch(
          sacksProvider(
            StockQuery(sourceId: sourceId, stockDate: query.stockDate),
    ...
    // ★ **وترتيبٌ ثابتٌ بالرقم المتسلسل** — ⛔ **ولا ترتيبَ يتبع وصولَ
    //   التدفّقات**: ⟵ **فقائمةٌ تتبدّل مواضعُها بين إطارين تُفقِد المستخدم
    //   موضعَ إصبعه** (نفسُ علّة [countedIntakeListProvider]).
    compare: (SackCard a, SackCard b) =>
        a.documentNumber.compareTo(b.documentNumber),
  ),
);
