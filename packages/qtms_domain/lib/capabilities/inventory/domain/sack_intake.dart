...
/// ★★★ **اسمُ سطرِ الدفتر كما يُعرَض** — [`ADR-0007`] · [`DEBT-86`].
///
/// ═══════════════════════════════════════════════════════════════════════
/// ⛔⛔★★★ **ولماذا لا يُعرَض [itemName] وحدَه:** ★ **حركةُ الجونية تُخزِّن
/// الاسمَ **المجرَّد** في `itemName`** («عتود») ⛔ **والمركّبَ في `itemKey`**
/// («عتود - جونية رقم 1») — ⟹ ⛔⛔ **فجونيتان من نوعٍ واحد في يومٍ واحد
/// تُعرَضان سطرين متطابقين لا يميّزهما شيء.**
///
/// ⛔⛔★★★ **وهو حرفياً الخيارُ 2 الذي رفضه [`ADR-0007`]:** «**المستخدم لا
/// يرى الفرق في قوائم الاختيار — يرى «بطوة معالم أحمر» مرتين بلا تمييز
/// فيختار الخطأ**»، ★ **والخيارُ المعتمَد وعد بأن «التمييز مرئي في كل قائمة
/// اختيار وكل شاشة مخزون»** — ⟵ **والمفتاحُ المركّب هو ذلك الاسم نفسُه.**
///
/// ★★ **والقاعدةُ مقيسةٌ من بناء [sackCompositeItemName] لا مُخمَّنة:**
/// **المركّبُ يبدأ بـ`'{الاسم} - '` حرفياً** — ⟹ **فما وافق البادئةَ مركّبٌ
/// يُعرَض بتمامه**، ⛔ **وما خالفها مفتاحُ سجلِ نوعٍ (`ITM-0002`) فيُعرَض
/// اسمُه المخزَّن** — ★ **وهو حالُ الوارد عدداً** (`BR-M6-10`).
///
/// ⚠️ **ولا قراءةَ كتالوجٍ هنا** — ★ **دالةٌ خالصةٌ من الحقلين المخزَّنين**:
/// ⟵ **فتصدُق في السحابة والتطبيق والتقارير بلا استعلامٍ ثانٍ**، ⛔ **ولا
/// تتبدّل بتبدّل سجلٍّ لم يكن مصدرَها أصلاً.**
/// ═══════════════════════════════════════════════════════════════════════
String ledgerItemDisplayName({
  required String itemKey,
  required String itemName,
}) {
  final String name = itemName.trim();
  if (name.isEmpty) return itemKey;
  return itemKey.startsWith('$name - ') ? itemKey : name;
}
