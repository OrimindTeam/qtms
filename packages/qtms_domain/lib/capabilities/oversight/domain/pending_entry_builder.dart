...
/// ★★★ هل **حان وقتُ** بند «الوزن الضائع لم يُؤكَّد»؟ — `OQ-002` (الخيار ب ·
/// 2026-09-17) · `FR-M7-19` · `CR-013`.
///
/// ═══════════════════════════════════════════════════════════════════════
/// ⛔⛔★★★ **والشرطُ حدثٌ لا مهلة:** ★ **البند لا يُكتب ما دام [stockDate]
/// هو يومَ المخزون الجاري** — ⟵ **فجونيةٌ المستخدمُ في وسط إدخال أنواعها
/// يبقى فيها وزنٌ غير مفسَّر بحكم الحال لا بحكم الإهمال**، ⛔ **ومطالبتُه
/// بتأكيده وهو يكتبه ضوضاءٌ متكرّرة تُضعف مركزاً غرضُه الملاحقة** (`GR-50`).
/// ★ **وبعد تغيّر تاريخ المخزون صار الوزنُ غيرَ المفسَّر متروكاً فعلاً**،
/// ⟵ **فيُطالَب به.**
///
/// ⛔⛔★★★ **ولا رقمَ مهلةٍ هنا ولا ساعة** — ★ **الحدُّ هو تغيّرُ اليوم
/// التقويمي نفسُه**: ⟵ **حدثٌ قائمٌ في النظام أصلاً يقيسه [today] المقروءُ
/// من المنصّة داخل المعاملة** (`GR-54`)، ⛔ **لا مهلةٌ تُختلَق** (مبدأ «لا
/// اختلاق» — `OQ-002` الخيار ج مرفوضٌ بنصِّه) ⛔ **ولا مسارُ كتابةٍ مجدول**
/// (`pending-entries-design.md` §12.2: **البند يُكتب في معاملة مستنده**).
///
/// ⚠️ **و[today] `null` تعني «لم يصل يومُ المنصّة»** — ⟵ **فلا حكمَ يُبنى
/// عليه**: ★ **نظيرُ `isAgedRemainder` حرفياً**، ⛔ **ولا رجوعَ إلى ساعةٍ
/// محلية** ⛔ **ولا ظهورٌ بالشكّ**: ★ **والبندُ يظهر عند أوّلِ عمليةٍ يصل
/// فيها اليوم** — ⟵ **فالغيابُ يؤجّل ولا يُلغي.**
/// ═══════════════════════════════════════════════════════════════════════
bool isSackLostWeightDue({
  required CalendarDay stockDate,
  required CalendarDay? today,
}) =>
    today != null && stockDate.compareTo(today) < 0;
...
///
/// ⏳★★★ **وله وحدَه شرطُ توقيتٍ ثانٍ — [isSackLostWeightDue]** (`OQ-002`
/// الخيار ب · `CR-013`): ★ **لا يُكتب قبل تغيّر تاريخ المخزون**،
/// ⛔ **والحقلان الآخران بلا شرطِ توقيتٍ إطلاقاً** — ⟵ **ضريبةٌ لم تُدخَل
/// وأنواعٌ لم تُدخَل نقصٌ من لحظته** ⛔ **لا حالةُ عملٍ جارٍ.**
/// ═══════════════════════════════════════════════════════════════════════
PendingEntrySet describeSackPending({
  ...
  // ★★★ **يومُ المنصّة كما قيس داخل المعاملة** — ⛔ **لا ساعةُ الحاوية ولا
  //    قيمةٌ من الجهاز** (`GR-54`): ⟵ **ومنه وحده يُعرَف هل تغيّر تاريخُ
  //    المخزون** ([isSackLostWeightDue]).
  required CalendarDay? today,
  ...
  if (!isCancelled) {
    ...
    // ⏳★★★ **وشرطُ التوقيت مع شرط الحالة معاً لا أحدُهما** — `OQ-002`
    //    (الخيار ب): ⟵ **فالوزنُ غيرُ المفسَّر في يوم مخزونه عملٌ جارٍ**
    //    ⛔ **لا نقصٌ متروك**، ★ **وبعد تغيّر التاريخ صار متروكاً فعلاً.**
    if (weightState == SackWeightState.unexplained &&
        isSackLostWeightDue(stockDate: stockDate, today: today)) {
      drafts.add(
        _sackDraft(
          sackId: sackId,
          sourceId: sourceId,
