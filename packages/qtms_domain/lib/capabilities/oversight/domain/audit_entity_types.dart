...
/// ★ سند الخصم — `M13` (`WU-013`).
///
/// ⚠️ **ومعرّفه رقم المستند `DSC-YYYYMMDD-####`** — ★ **بخلاف
/// [distributionEntityType] الذي معرّفه مركّب**: ⟵ **لأن `discounts`
/// مفتاحُها `documentNumber` نفسُه** (`schema/discounts.md`)، ★ **والسجل
/// السياقي 🕘 يستعلم بما يُفتَح به المستند فعلاً** (`FR-M18-10`).
///
/// ⛔⛔★★ **ونوعٌ مستقلٌّ عن [receiptEntityType] قطعاً** — ★ **رغم تطابق
/// آليتهما**: ⟵ **لأن `FR-M15-06-أ` يفرض التمييز بين ما دفعه المقوت نقداً
/// وما أسقطه المالك عنه**، ⛔ **وقيدُ تدقيقٍ بنوعٍ واحد كان يجعل سجل
/// التدقيق نفسَه عاجزاً عن التفريق بينهما.**
const String discountEntityType = 'discount';

/// المستخدم — `M1`.
/// ★★ **سحبية** — `M22` (`WU-014`).
///
/// ⛔⛔★★ **ونوعان لا نوعٌ واحد بحقل تمييز** — `FR-M22-03` · `GR-43`:
/// ⟵ **فمرشِّحُ سجل التدقيق نفسُه يجب أن يفصل السجلَّين**، ★ **وإلا رأى مَن
/// يملك «عرض الخرجيات» وحدها عناوينَ قيودِ سحبيات المالك في القائمة.**
const String withdrawalEntityType = 'withdrawal';

/// ★★ **خرجية** — `M22` (`WU-014`) — راجع [withdrawalEntityType].
const String expenseEntityType = 'expense';
...
const List<String> auditEntityTypes = <String>[
  ...
  //    لا يحرسها شيء.**
  receiptEntityType,
  // ★★ **أُضيفا في `WU-014` (2026-09-01) — ★ ولكلٍّ كاتبٌ فعليٌّ في نفس
  //    الزيادة:** `outflow.dart` **يكتب `entityType` من [OutflowLedgerType]
  //    المخزَّن** ⛔ **لا من الحمولة** — ⟵ **فالقيمتان لهما قيدٌ حقيقي**،
  //    ★ **وقاعدةُ هذا الملف مُستوفاة** («لا تُضاف قيمة بلا كاتبٍ لها فعلاً»).
  ...
  cashSaleEntityType,
  // ⚠️★★ **أُضيف في `WU-013` — ★ وله كاتبٌ فعلي في نفس الزيادة:**
  //    ★ **`discount.dart` يكتبه في `entityType` لكل إنشاءٍ وتعديلٍ وإلغاء**،
  //    ⟵ **وحارسُ `planExportLog` يقرأ هذه القائمةَ نفسَها** (`IQ-032`):
  //    ⛔ **فبلا هذا السطر يُرفَض تصديرُ كل سند خصم** (`FR-M13-12`) —
