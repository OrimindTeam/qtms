...
final class AuditLogEntryCard {
  ...
  /// ★★★ **بريدُه مُثبَّتاً وقت الحدث كذلك** — `AM-012` §3.1 و§3.2.
  ///
  /// ⛔⛔★★★ **و`null` تعني «الحقل غائبٌ في هذا القيد» لا «لا بريد له»:**
  /// ★ **كلُّ قيدٍ كُتب قبل 2026-09-02 يحمله غائباً** — ⟵ **والسجلُّ للإضافة
  /// فقط ولا يُهاجَر** (`schema/audit-log.md`)، ⛔ **فلا ملءَ بأثرٍ رجعي.**
  ///
  /// ★★ **وأثرُه في العرض مُحدَّد:** ⟵ **يسقط سطرُ البريد وحدَه**،
  /// ⛔ **ولا يُكتب «لا قيمة» في موضع هوية** — ★ **بخلاف صفوف «قبل/بعد»
  /// حيث الغيابُ نفسُه معلومة** (`_valueLabel`).
...
List<AuditFieldChange> describeAuditChanges(AuditLogEntryCard entry) {
  // ⛔⛔★★★ **والإنشاءُ بلا صفوفٍ إطلاقاً** — `AM-012` §3.1 (2026-09-02).
  //
  // ★★★ **ولماذا الإنشاءُ وحدَه — وهو تمييزٌ دلاليٌّ لا تقليلُ ضجيج:**
  //    ★ **«قبل/بعد» تُجيب سؤالاً واحداً: ما الذي تغيّر؟** ⟵ **وفي الإنشاء
  //    لا شيءَ تغيّر** — ★ **بل وُجد**: ⛔ **فكلُّ صفٍّ يقول «لا قيمة ⟵ س»**،
  //    ⟵ **وهو تكرارُ محتوى المستند نفسِه بصيغةِ تغييرٍ لم يقع.**
  //
  // ⛔⛔ **والمعلومةُ لا تضيع:** ★ **المستندُ المُنشأ نفسُه مفتوحٌ من شاشته
  //    بكل حقوله**، ⟵ **والقيدُ يبقى بفعله ومُنفِّذه ووقته وهدفه** — ★ **وهي
  //    أركانُ «مَن فعل ماذا ومتى»** (`FR-M18-05`) ⛔ **ولم يسقط منها شيء.**
  //
  // ⚠️⚠️★★ **و`valuesAfter` تبقى مكتوبةً في القاعدة كما هي** — ★ **هذا
  //    إسقاطُ عرضٍ لا إسقاطُ تخزين**: ⟵ **فسجلُّ التدقيق يحتفظ بلقطة
  //    الإنشاء كاملةً للتحقيق**، ⛔ **والشاشةُ وحدَها هي ما تتغيّر.**
  if (entry.action == AuditAction.create) {
    return const <AuditFieldChange>[];
  }
