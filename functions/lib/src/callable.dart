...
final class CallableError {
  ...
  /// ★★★ **جردٌ مفتوحٌ على نفس المصدر ونفس اليوم** — `ERR_STOCK_006`
  /// (`FR-M16-06` · `BR-M16-05`).
  ///
  /// ⚠️ **والمقياس «مسوّدةٌ قائمة» لا «مستندٌ قائم»** — ⟵ **فجردٌ اعتُمد أو
  /// أُلغي لا يمنع جرداً ثانياً في اليوم نفسه**: ★ **العدُّ قد يُعاد**،
  /// ⛔ **والمنعُ المطلق كان يحبس اليومَ بعد أول خطأ.**
  ///
  /// ⛔⛔★★ **ولا يُعاد استعمال `ERR_CONC_001` هنا** — ★ **نصُّه «تم تعديل
  /// البيانات من مستخدم آخر. يرجى إعادة المحاولة»**: ⟵ **وإعادةُ المحاولة
  /// لن تُجدي شيئاً** — ★ **العلاجُ إتمامُ الجرد المفتوح أو إلغاؤه**،
  /// ⛔ **ورسالةٌ تُرشد لفعلٍ لا ينفع أسوأ من رسالةٍ عامة.**
  ///
  /// ✅★★ **ونصُّ عرضه اعتمده صاحب المشروع صراحةً (2026-09-05 · `IQ-042`)**
  /// — `error-codes-catalog.md` §3 القاعدة 5: ⟵ **«يوجد جرد مفتوح لهذا
  /// المصدر في هذا اليوم — اعتمده أو ألغِه قبل بدء جرد جديد.»**، ★ **وموضعُه
  /// `lib/core/messages/error_messages.dart`** ⛔ **لا هنا** (**لا نصَّ عربياً
  /// في أي استجابة — راجع ترويسة الملف**).
  static const CallableError stocktakeInProgress = CallableError(
    code: 'ERR_STOCK_006',
    status: 'FAILED_PRECONDITION',
    httpStatus: 400,
  );

  /// ★★★ **حالة الجرد لا تسمح بهذه العملية** — `ERR_STOCK_007` (`FR-M16-09`).
  ///
  /// ⛔⛔ **ورمزٌ واحدٌ للحالتين معاً** — **اعتمادُ ما ليس مسوّدةً** و**تعديلُ
  /// ما ليس معتمداً**: ★ **بنفس منطق `supplierRequired` حرفياً**
  /// (`error-codes-catalog.md` §3 القاعدة 5) — ⟵ **الفعلُ المُرشَد إليه واحد:
  /// افتح المستند وانظر حالته**، ⛔ **ورمزٌ ثالثٌ كان يضاعف نصّاً ينتظر
  /// اعتماداً بلا أن يُغيِّر ما يفعله المستخدم.**
  ///
  /// ✅★★ **ونصُّ عرضه اعتمده صاحب المشروع صراحةً كذلك** (2026-09-05 ·
  /// `IQ-042`): ⟵ **«حالة مستند الجرد لا تسمح بهذه العملية — حدّث الشاشة
  /// وأعد المحاولة.»**
  static const CallableError stocktakeNotDraft = CallableError(
    code: 'ERR_STOCK_007',
    status: 'FAILED_PRECONDITION',
    httpStatus: 400,
  );

  /// ★ **تعديلُ جردٍ لم يُعتمَد بعد** — `ERR_STOCK_007` نفسُه.
  ///
  /// ★★ **واسمان لرمزٍ واحد عمداً** — ⟵ **فموضعُ الرفض في `planStocktake`
  /// يُقرأ بمعناه** (`stocktakeNotApproved` **لا** `stocktakeNotDraft`)،
  /// ⛔ **والمستخدمُ يرى رسالةً واحدة** (راجع [stocktakeNotDraft]).
  static const CallableError stocktakeNotApproved = stocktakeNotDraft;
  ...
  // ══════════════════════════════════════════════════════════════════════
  // البيع النقدي المباشر (`WU-012`) — ★★ **ورمزٌ قائمٌ في الكتالوج §`PRICE`**
  //
  // ⛔★★ **ولا رمزَ مُخترَع:** `error-codes-catalog.md` §3 القاعدة 4 والقاعدة 5
  //    (**نصّ أي رسالة جديدة يعتمده صاحب المشروع صراحةً**، حسم `IQ-016`) —
  //    ⟵ **فالزيادة تستهلك الكتالوج ولا تُوسّعه.**
  // ══════════════════════════════════════════════════════════════════════

  /// ⛔⛔★★★ **سعرٌ دون الحد الأدنى بلا مفتاح التجاوز** — `ERR_PRICE_002`
  /// (`FR-M11-05` · `GR-34`).
  ///
  /// ★★ **والصلاحيةُ وحدها هي الحارس** (`cashSaleBelowMinimum`) —
  /// ⛔ **و`ERR_PRICE_003` («سبب التجاوز مفقود») لم يعد يُصدَر** بـ`ADR-0020`:
  /// ⟵ **فالسببُ اختياريٌّ ولا يُرفَض بيعٌ لغيابه.**
  ///
  /// ⚠️ **ونوعٌ بلا حدٍّ مسجَّل لا يقع تحت هذا الرمز إطلاقاً** (`FR-M11-06`) —
  /// ★ **يُباع بتنبيه ويبقى في مركز الإدخالات المعلّقة.**
  /// ★★★ **سعرٌ مفقود في بيعٍ نقدي** — `ERR_PRICE_004` (`FR-M11-04`).
  ///
  /// ⛔⛔★★ **ورمزٌ مستقلٌّ عن `invalidArgument` عمداً:** ★ **نصُّه يقول
  /// للمستخدم الحقلَ الذي عليه ملؤه** («البيع النقدي يتطلب إدخال سعر لكل
  /// نوع») — ⟵ **بينما «طلبٌ غير صالح» لا يُرشد إلى فعل.**
  static const CallableError cashPriceMissing = CallableError(
    code: 'ERR_PRICE_004',
    status: 'INVALID_ARGUMENT',
    httpStatus: 400,
  );

  static const CallableError belowMinimumCashPrice = CallableError(
    code: 'ERR_PRICE_002',
    status: 'FAILED_PRECONDITION',
    httpStatus: 400,
  );
  ...
  /// ★★ **مبلغ الخصم يتجاوز المتبقي على الضمار** — `ERR_DIST_005`
  /// (`FR-M13-04` · `BR-M13-02`).
  ///
  /// ⛔⛔★★ **ورمزٌ مستقلٌّ عن [receiptExceedsDebt]** — ★ **رغم تطابق
  /// الشرط**: ⟵ **لأن رسالتيهما مختلفتان في `error-codes-catalog.md`**
  /// («**مبلغ الخصم**» مقابل «**المبلغ الواصل**»)، ⛔ **ورسالةٌ تقول
  /// «الواصل» في شاشة خصمٍ تُربك المستخدم في أدقّ تمييزٍ في النظام**
  /// (`FR-M15-06-أ`).
  static const CallableError discountExceedsDebt = CallableError(
    code: 'ERR_DIST_005',
    status: 'FAILED_PRECONDITION',
    httpStatus: 400,
  );

  /// ⛔⛔★★★ **حقلُ فائضٍ في سند خصم** — `ERR_DIST_009` (`FR-M13-05` ·
  /// `BR-M13-03` · `AT-35`).
  ///
  /// ⛔⛔★★★ **وهذا حارسٌ للمسار السحابي لا للشاشة** — ★ **فالحقلُ غيرُ
  /// موجودٍ في الشاشة أصلاً**: ⟵ **ووصولُه يعني أن الطلب أُرسل من خارج
  /// التطبيق**، ★ **ونصُّ `FR-M13-05` يفرض رفضَه صراحةً** ⛔ **لا تجاهلَه:**
  /// ⟵ **فتجاهلُه الصامت كان يقبل سنداً ادّعى مُرسِلُه فائضاً ثم لا يجده.**
  static const CallableError discountSurplusRejected = CallableError(
    code: 'ERR_DIST_009',
    status: 'INVALID_ARGUMENT',
    httpStatus: 400,
  );

  /// ⛔⛔★★★ **مصدرٌ مفقود في سحبيةٍ أو خرجية** — `ERR_OUT_001`
  /// (`FR-M22-02` · `GR-42` · `E-28`).
  ///
  /// ⛔⛔★★ **ورمزٌ مستقلٌّ عن `ERR_CALL_400` عمداً:** ★ **نصُّه في الكتالوج
  /// يقول الشرطَ كاملاً** («**لا يمكن حفظ السحبية أو الخرجية بلا مصدر —
  /// حتى سحبيات المالك**») — ⟵ **وهو بالضبط ما يظنّه المستخدم استثناءً
  /// لنفسه**، ★ **فالرسالةُ تُغلق الظنَّ صراحةً** ⛔ **بينما «طلبٌ غير صالح»
  /// كانت ستتركه يُعيد المحاولة بلا مصدرٍ مرةً أخرى.**
  static const CallableError outflowSourceMissing = CallableError(
    code: 'ERR_OUT_001',
    status: 'INVALID_ARGUMENT',
    httpStatus: 400,
  );

  /// ⛔⛔★★★ **تسعيرُ قاتٍ بلا مفتاحه** — `ERR_OUT_002` (`FR-M22-08`).
  ///
  /// ★★ **ورمزٌ مستقلٌّ عن `ERR_AUTH_001` عمداً — بنفس علّة `ERR_PRICE_001`
  /// حرفياً:** ⟵ **المُنفِّذ يملك `withdrawalCreate` فعلاً ويُسمح له بالحفظ**،
  /// ⛔ **والممنوعُ حقلُ السعر وحده** — ★ **ونصُّ الكتالوج يقول له ماذا
  /// يحدث بدلاً منه** («**سيُسعَّر لاحقاً**») ⟵ **فلا يظنّ السندَ مرفوضاً.**
  ///
  /// ⛔⛔★★ **والمفتاحُ يتبع السجل** (`GR-43`): **`withdrawalQatPriceNow`
  /// للسحبية · `expenseQatPriceNow` للخرجية** — ★ **ولا يُغني أحدهما عن
  /// الآخر.**
  static const CallableError outflowPricingNotAllowed = CallableError(
    code: 'ERR_OUT_002',
    status: 'PERMISSION_DENIED',
    httpStatus: 403,
  );
