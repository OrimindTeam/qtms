...
/// ★★★ **مرجعُ الجونية لكل مفتاحٍ — مقروءاً من الدفتر** (`DEBT-86`).
///
/// ═══════════════════════════════════════════════════════════════════════
/// ⛔⛔★★★ **ولماذا يُقاس ولا يُصدَّق من الحمولة:** `ADR-0007` القاعدة 5
/// «**كل حركة خروج تحمل مرجع الجونية**» — ★ **وعليه وحده يُبنى فهرسُ تفكيك
/// سعر الجونية** (`FR-M10-14` · `A-13`)، ⟹ ⛔⛔ **وغيابُه عن سطرٍ من جونية
/// يُسقِط الجونيةَ من الاحتساب بصمت** — ★ **ولا خطأَ يُرفَع ولا رقمَ يبدو
/// شاذاً**: **يقلّ إيرادُ الرعوي فحسب.**
///
/// ⚠️⚠️★★★ **وهذا ما كان سيقع لحظةَ فتحِ [`DEBT-86`]:** ★ **التطبيق لم
/// يُرسِل `sackId` في أي سطرٍ قطّ** (⛔ **ولا حقلَ له في أي شاشة**) — ⟵ **فما
/// إن صار المفتاحُ المركّب قابلاً للاختيار حتى صارت كلُّ حركةِ خروجٍ منه
/// بلا مرجعِ جونية.** ★ **والعلاجُ أن يُشتقّ في المعاملة من حركةِ الدخول
/// نفسِها** ⛔ **لا أن يُضاف حقلٌ سادسٌ إلى ثلاث شاشات يُنسى في الرابعة.**
///
/// ★★ **والمفتاحُ المركّب لجونيةٍ واحدة تعريفاً** (`{النوع} - {الرعوي} -
/// جونية رقم N`) — ⟵ **فأولُ حركةٍ حيّةٍ تحمله تكفي**، ⛔ **ولا التباسَ بين
/// جونيتين على مفتاحٍ واحد.** ★ **والمفتاحُ المجرَّد (الوارد عدداً) لا
/// حركةَ له تحمله** ⟹ **فيبقى غائباً** — **وهو الصواب** (`BR-M6-10`).
///
/// ⛔★★ **ولا يُشتقّ شيءٌ من حركةٍ ملغاة** — ★ **بنفس حارس [withLedgerItems]**:
/// ⟵ **الملغاةُ ليست مصدرَ هوية كما أنها ليست مصدرَ رصيد** (`A-14`).
/// ═══════════════════════════════════════════════════════════════════════
Map<String, String> ledgerSackIds({
  required TransactionReads reads,
  required Iterable<String> itemKeys,
}) {
  final Map<String, String> sackIds = <String, String>{};
  for (final String itemKey in itemKeys) {
    if (sackIds.containsKey(itemKey)) continue;
    for (final Map<String, Object?> document in reads.matchedDocuments(itemKey)) {
      if (document['isCancelled'] == true) continue;
      final Object? sackId = document['sackId'];
      if (sackId is String && sackId.isNotEmpty) {
        sackIds[itemKey] = sackId;
        break;
      }
    }
  }
