...
void main() {
  group('★★ IQ-019 — كل مسار معروف يرتبط بمعالجٍ واحد محدد', () {
    ...
    test('★★ وكلُّ العمليات موجودة — ⛔ ولا عملية سقطت من الجدول', () {
      ...
      expect(
        ...
        <String>{
          ...
          // ★★ **ثلاث عمليات** — ⛔⛔ **ولا رابعةَ لدفتر ذمم:** `FR-M11-03`
          //    («**لا يُنشئ أي حركة في دفتر حسابات المقاوته**») — ⟵ **وهو
          //    فارقُها الجوهري عن التوزيع.**
          'createCashSale',
          'amendCashSale',
          'cancelCashSale',
          // ── الخصومات (`WU-013`) — `api-overview.md` §3.1 ──
          // ★★ **ثلاث عمليات** — ⛔⛔★★★ **ولا رابعةَ لإيداعٍ بنكي:**
          //    `FR-M13` §2 (**«لا ينطبق — لا نقد فيه»**) — ⟵ **وهو فارقُها
          //    الجوهري عن المقبوضات**، ★ **مع أن آليتهما واحدة.**
          //
          // ⛔⛔★★★ **ومسارات مستقلة عن مسارات القبض قطعاً** (`FR-M15-06-أ`):
          //    ⟵ **فما دفعه المقوت نقداً وما أسقطه المالك عنه رقمان
          //    مختلفان**، ⛔ **ومسارٌ واحد بعَلَمٍ كان يجعل الخلطَ حقلاً
          //    منسيّاً في حمولة.**
          'createDiscount',
          'amendDiscount',
          'cancelDiscount',
          // ── الإرسال والتصدير (`WU-010`) — `api-overview.md` §3.1 ──
          // ⛔★★ **وعمليةٌ واحدة لا اثنتان:** ★ **الإرسال إجراء واجهة**
          //    ⛔ **ولا يُسجَّل إطلاقاً** (`FR-M20-15` · `AT-64`) — ⟵ **فلا
          //    مسارَ له هنا**، ★ **والتصدير وحده يُسجَّل** (`FR-M19-04`).
          'logExport',
          // ── إعادة البناء بأثر رجعي (`WU-021`) — `api-overview.md` §3.1 ──
          // ⛔⛔★★ **إجراءٌ تشغيليٌّ لا يُستدعى من التطبيق** — ★ **بنفس صنف
          //    `bootstrapOwnerPermissions`** (`RB-rebuild-summaries.md`):
          //    ⟵ **وحارسُه هويةُ المالك المسجَّل وحدها** ⛔ **لا مفتاحٌ من
          //    الكتالوج** (`permissions-catalog.md` §3: **«كتابةُ الأرصدة
          //    والملخصات» ليست صلاحيةً ولا مفتاحَ لها**).
          'rebuildDayRetroactively',
          // ── السحبيات والخرجيات (`WU-014`) — `api-overview.md` §3.1 ──
          // ⛔⛔★★★ **وثلاثٌ لسجلَّين ⛔ لا ستٌّ لكل سجلٍّ ثلاث:** ★ **آليةُ
          //    السجلَّين واحدةٌ تماماً والفارقُ في الصلاحية والتصنيف**
          //    (`outflow-design.md` §2) — ⟵ **و`ledgerType` وسيطٌ إلزاميٌّ
          //    يختار المفتاح** (`outflowPermission`)، ⛔ **وستُّ عملياتٍ
          //    كانت تُكرِّر الكودَ نفسَه ثم تفترق نسختاه عند أول تعديل.**
          // ⛔⛔ **والفصلُ الأمني قائمٌ كاملاً رغم المسار الواحد** (`GR-43`):
          //    ★ **ثمانيةُ مفاتيح** · **وقاعدةُ الحماية تقرأ `ledgerType`** ·
          //    ★★ **والسجلُّ المخزَّن هو الحَكَم في التعديل والإلغاء.**
          'createOutflow',
          'amendOutflow',
          'cancelOutflow',
          // ── الإتلاف (`WU-020`) — `api-overview.md` §3.1 ──
          // ⛔⛔★★★ **وثلاثُ عملياتٍ بثلاثة مفاتيح مستقلة** (`disposalCreate`
          //    · `disposalAmend` · `disposalCancel`) — ★ **الكتالوج §2.4
          //    وجدولُ التعديل والإلغاء**: ⟵ **فمن يسجّل إتلافاً لا يُلغيه.**
          // ⛔⛔ **ولا رابعةَ لتسعير:** ★ **لا قيمة للمُتلَف أصلاً**
          //    (`FR-M8-16`) — ⟵ **وهو فارقُه الجوهري عن السحبية.**
          'createDisposal',
          'amendDisposal',
          'cancelDisposal',
          // ── الجرد (`WU-022`) — `api-overview.md` §3.1 ──
          // ⛔⛔★★★ **وأربعُ عملياتٍ لا ثلاث — والسببُ متطلبٌ صريحٌ لا ذوق:**
          //    `FR-M16-03` **يُجمِّد الرصيد «لحظة بدء الجرد»** و`FR-M16-09`
          //    **يجعل الاعتماد صلاحيةً مستقلة** — ⟵ **فمرحلتان لا واحدة**،
          //    ⛔ **وعمليةٌ واحدة كانت تُلغي معنى النصّين معاً.**
          // ⛔⛔ **وأربعةُ مفاتيح مستقلة** (`stocktakeWrite` ·
          //    `stocktakeApprove` · `stocktakeAmend` · `stocktakeCancel`):
          //    ⟵ **فمن يعدّ البضاعة لا يُقرّ تسويةَ الدفتر عليها.**
          'startStocktake',
          'approveStocktake',
          'amendStocktake',
          'cancelStocktake',
