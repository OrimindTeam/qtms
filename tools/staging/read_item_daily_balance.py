...
item_key = sys.argv[2] if len(sys.argv) > 2 else 'ITM-0003'
compact = sys.argv[3] if len(sys.argv) > 3 else '20260901'
# ★★ **واليومُ في المفتاح مضغوطٌ `YYYYMMDD`** — `CalendarDay.format()`:
#    ⛔ **لا `YYYY-MM-DD`** (`naming-conventions.md` §4 — **الترتيبُ والصيغةُ ملزمان**).
doc_id = '%s_%s_%s' % (source_id, item_key, compact)
...
# ── ② بطاقةُ الرصيد نفسُها ─────────────────────────────────────────────
status, raw = get('%s/item_daily_balances/%s' % (DB, doc_id), token)
print('② GET item_daily_balances/%s -> %s' % (doc_id, status))
if status != 200:
    print(safe(raw)[:400])
    sys.exit(1)
