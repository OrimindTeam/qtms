# -*- coding: utf-8 -*-
"""قراءةُ مستند إتلافٍ من التجريبية **حقلاً بحقل** — `WU-020` (`FR-M8-16`).

★ **التشغيل من جذر المستودع:**

    python tools/staging/verify_disposal_write.py DSP-20260904-0001

═══════════════════════════════════════════════════════════════════════════
⛔⛔★★★ **ولماذا لا تكفي رسالةُ الشاشة «✅ سُجِّل الإتلاف»:** ★ **الشاشةُ
تقرأ ردَّ العملية لا المخزَّن** — ⟵ **وأربعةُ أحكامٍ لا تُرى منها إطلاقاً:**

  ① ⛔⛔ **لا حقلَ ماليٍّ في المستند ولا في سطوره** (`FR-M8-16`).
  ② ★★ **حركةُ الدفتر مَوْسومةٌ `disposal`** — ⟵ **وهو ما يُخرِجها من سعر
     الجونية ومن المبيعات** (`design-overview.md` §2.2).
...
(`IQ-024` · `WU-008`): ⟵ **والتصفيةُ برقم المستند تقع *بعد* الجلب.**

⚠️ **ويدخل بحساب QA لا المالك** — ★ **فالمقصود إثباتُ ما يراه المستخدم
الحقيقي بصلاحياته وشرطِ قراءته.**
═══════════════════════════════════════════════════════════════════════════
"""
...
number = sys.argv[1] if len(sys.argv) > 1 else 'DSP-20260904-0001'
...
# ── ② المستند نفسُه ────────────────────────────────────────────────────
status, raw = get('%s/disposals/%s' % (DB, number), token)
print('② GET disposals/%s -> %s' % (number, status))
if status != 200:
    print(safe(raw)[:400])
    sys.exit(1)
doc = {k: plain(v) for k, v in json.loads(raw)['fields'].items()}
for key in ('documentNumber', 'sourceId', 'sourceName', 'stockDate', 'status',
            'totalPieces', 'totalWeight', 'lineCount', 'reason', 'createdBy'):
    if key in doc:
...
if moves is None:
    ...
    for name, f in moves:
        print('   ·', name, '→', json.dumps(
            {k: f.get(k) for k in
             ('itemKey', 'itemName', 'direction', 'quantity', 'balanceAfter',
              'movementTag', 'sourceDocType', 'stockDate', 'sackId',
        ...
        print('     ★ الوسم =', f.get('movementTag'),
              '✅' if f.get('movementTag') == 'disposal' else '⛔⛔ مخالفة!')

# ── ④ ⛔⛔ ولا قيدَ في دفتر المقاوته ولا في دفتر الرعوي — `A-15` · `GR-29` ──
for ledger, rule in (('dealer_ledger', 'GR-29'), ('supplier_ledger', 'A-15')):
...
line_keys = [l.get('itemId') for l in (doc.get('lines') or []) if l.get('itemId')]
stock_day = (doc.get('stockDate') or '')[:10].replace('-', '')
for item_key in line_keys:
    balance_id = '%s_%s_%s' % (source_id, item_key, stock_day)
    status, raw = get('%s/item_daily_balances/%s' % (DB, balance_id), token)
...
# ── ⑥ قيدُ التدقيق — الفعلُ «إتلاف» باسمه (`FR-M18-08`) ────────────────
status, raw = post('%s:runQuery' % DB, {
    'structuredQuery': {
