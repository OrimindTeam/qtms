...
### 6.1 ⛔⛔★★★ حزمةُ الجهاز الحقيقي تُبنى بأمرها — ⛔ ولا تُنسَخ حزمةُ المحاكي

⛔⛔★★★ **`flutter run -d <المحاكي>` يكتب في المسار نفسِه الذي يكتبه
`flutter build apk`:**
...
⚠️★★ **لكنّه يبني معماريةَ الجهاز المتصل وحدَها** — ⟹ **فحزمةُ المحاكي
`Pixel_6_API_36` تحمل `lib/x86_64/libflutter.so` لا غير**، ⛔ **وتُثبَّت على
هاتف `arm64-v8a` بنجاحٍ تامّ ثم تنهار قبل أول إطار**:

```text
com.getkeepsafe.relinker.MissingLibraryException: Could not find
'libflutter.so'. Looked for: [arm64-v8a, armeabi-v7a, armeabi],
but only found: [x86_64].
```

⛔⛔★★★ **ولا أثرَ لها في شاشة المستخدم إلا «تستمر عملية … في التوقف»** —
★ **ولا في `flutter analyze` ولا في `flutter test` ولا في أي بوابةٍ آلية**:
★★ **الحارسُ الوحيد فحصُ محتوى الحزمة قبل تسليمها** (`DEBT-94` · رُصد فعلياً
2026-09-05 على `SM-G925V` · أندرويد 7.0 · API 24).

**① ★★★ حزمةُ التسليم — شاملةُ المعماريات** (⛔ **هي وحدَها ما يُسلَّم لعميلٍ
أو يُجرَّب على هاتفٍ مجهول المعمارية**): ⛔⛔ **بلا `--target-platform` إطلاقاً**
— ★ **والعَلَمُ في الأمر نفسِه لا فوقه** (`IQ-041` · `AM-014`):

```bash
set TEMP=E:\Projects\qtms\.gradle-tmp && set TMP=E:\Projects\qtms\.gradle-tmp && flutter build apk --debug --flavor staging --dart-define-from-file=config/qtms-public-defines.env
```

| الصيغة | تعمل على | متى تُستعمَل |
|---|---|---|
| ★★★ **شاملة** (بلا `--target-platform`) | `arm64-v8a` + `armeabi-v7a` + `x86_64` — ⟹ **كلُّ هاتف أندرويد 7.0+ والمحاكي معاً** | ✅ **التسليمُ للعميل · تجربةُ أي هاتفٍ حقيقي** |
| `--target-platform android-arm64` | `arm64-v8a` وحدَها | ★ هاتفٌ **تأكّدتَ** من معماريته — أصغر حجماً |
| `--split-per-abi` | حزمةٌ لكل معمارية | ★ التوزيعُ بالحجم الأدنى — **ويجب اختيارُ الملف الصحيح للجهاز** |

> ⛔⛔★★ **و`armeabi-v7a` ليست ترفاً:** ★ **أجهزةُ أندرويد 7 كثيرةٌ منها 32-بت**
> — ⟹ **وحزمةُ `android-arm64` تنهار عليها بالخطأ نفسِه حرفياً.**
> ★ **وتُقرأ معماريةُ جهازٍ متصل بـ:** `adb shell getprop ro.product.cpu.abi`

**② ★★ والتحقّقُ قبل التسليم — سطرٌ واحد يقرأ الحزمة نفسَها** ⛔ **لا سجلَّ
البناء:**

```bash
unzip -l build/app/outputs/flutter-apk/app-staging-debug.apk | grep libflutter.so
```

⟹ ★★★ **يجب أن تظهر المعمارياتُ الثلاث في حزمة التسليم:**
...
⛔ **وظهورُ `x86_64` وحدَه يعني أن الحزمة حزمةُ محاكٍ — تُعاد بناءً ولا تُسلَّم.**

⛅★★★ **مقيسٌ حيّاً 2026-09-05 — حزمةٌ واحدة على الجهازين معاً:**
✅ `SM-G925V` (`arm64-v8a` · **أندرويد 7.0 · API 24**) — **٠ `FATAL EXCEPTION`**
· ✅ `emulator-5554` (`sdk_gphone64_x86_64` · `Pixel_6_API_36`) — **٠ `FATAL`
و٠ `MissingLibraryException`.**

> ⚠️★★ **وحزمةُ التصحيح الشاملة ضخمة (~199 ميغابايت)** — ★ **تحمل ثلاثةَ
> محرّكاتٍ و`libVkLayer_khronos_validation.so`**، ⟹ **وهي للتجربة لا للتسليم
> النهائي:** ★ **حزمةُ `--release` أصغر بمراتب** (§5).

### 6.2 ★★ موضعُ حزمة التسليم واسمُها — `dist/`

★★ **الحزمةُ المُسلَّمة تُنسَخ إلى `dist/` باسمٍ يحمل هويتها كاملة** ⛔ **ولا
تُشارَك من `build/` باسمها العام `app-staging-release.apk`** — ★ **فالاسمُ
العام لا يقول نكهةً ولا إصداراً ولا تاريخاً**، ⟹ **وحزمتان على درايف بالاسم
نفسِه لا يُفرَّق بينهما.**

```text
dist/qtms-<النكهة>-v<الإصدار>-b<البناء>-universal-<YYYYMMDD>.apk
dist/<الاسم نفسه>.apk.sha256
```

⛔ **و`dist/*` مستبعَدٌ في `.gitignore`** (★ **عدا `dist/README.md`**) —
★ **والعقدُ الكامل وقيودُ المستلِم فيه.**

⛅★★★ **مقيسٌ حيّاً 2026-09-05 على الملف المُسلَّم نفسِه** (⛔ **لا على مخرَج
البناء**): ✅ **ثلاثُ معماريات** · ✅ **`sha256sum -c` مطابقة** · ✅ **إقلاعٌ
على `SM-G925V` في `+1s333ms` بـ٠ `FATAL`** · ✅ **وشاشةُ الدخول بلا اعتمادِ QA
مملوء** ⟹ ★★ **فحارسُ `kDebugMode` في `staging_qa_credentials.dart` مُثبَتٌ
حيّاً في بناء الإصدار** ⛔ **لا مفترَضاً.**

> ⛔⛔★★ **وتوقيعُها شهادةُ `Android Debug`** (`TODO` قائمٌ في
> `android/app/build.gradle.kts`) — ★ **وهي محليةُ الجهاز**: ⟹ **حزمةٌ تالية
> من حاسوبٍ آخر تُرفَض بتعارض توقيع فوق المثبَّتة**، ⛔ **وهو ما يخالف §6
> «تُوقَّع بنفس الشهادة دائماً»** ⟹ ★★ **فشهادةُ إصدارٍ حقيقية شرطٌ قبل أول
> تسليمٍ إنتاجي.**
